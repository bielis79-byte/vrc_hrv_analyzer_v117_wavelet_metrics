...
st.set_page_config(page_title="VRC / HRV RRi Analyzer Pro v11.7", layout="wide")
...
def wavelet_temporal_metrics(rr):
    ...
    try:
        ...
        labels = np.argmax(bands_valid, axis=0)  # 0 VLF, 1 LF, 2 HF
...
| VLF_DOM_PCT, LF_DOM_PCT, HF_DOM_PCT | Dominancia temporal | % de puntos temporales donde esa banda es la mayor | HF alto = predominio respiratorio/vagal; LF alto = barorreflejo; VLF alto = regulación lenta. |
| EPISODES y TRANSITIONS | Episodios y cambios de régimen | Conteo/duración de dominios VLF/LF/HF y cambios entre ellos | Transiciones altas = movilidad; transiciones bajas = régimen fijo. |
...
def _interpret_metric(metric, value):
    ...
    if metric == "HF_DOM_PCT":
        if v >= 50:
            return "HF_DOM_PCT alto: predominio respiratorio/vagal durante gran parte de la ventana."
        if v < 20:
    ...
    if metric == "LF_DOM_PCT":
        if v >= 50:
            return "LF_DOM_PCT alto: predominio de oscilaciones LF/barorreflejas durante la ventana."
        if v < 20:
    ...
    if metric == "VLF_DOM_PCT":
        if v >= 50:
            return "VLF_DOM_PCT alto: mayor peso de regulación lenta; valorar termorregulación, fatiga, RAS, inflamación o no estacionariedad."
        if v < 20:
...
def _single_record_report(record_name, metrics_df, windows, rr=None):
    ...
    if any(c in metrics_df.columns for c in ["LF_WAV","HF_WAV","LF_HF_WAV"]):
        ...
        lines.append("")
        lines.append("Los valores LF_WAV, HF_WAV y LF_HF_WAV son resúmenes medios de una matriz tiempo-frecuencia. Para interpretar aparición/desaparición de HF, emergencia de LF o transiciones rápidas debe revisarse el scalogram de la pestaña HRV/exportación.")
        lines.append("")
...
st.title("VRC / HRV RRi Analyzer Pro v11.7")
